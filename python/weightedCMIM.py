...
class weightedCMIM():
    ...
    def fit(self, data, labels):
        """
        Fits a defined weightedCMIM filter to a given data set.
        ...
        weight_vector = self.weight_vector.values
        ...
        c_weight_vector = weight_vector.ctypes.data_as(c.POINTER(c.c_double))
        c_feature_scores = selected_features_score.ctypes.data_as(c.POINTER(c.c_double))

        self.libFSToolbox.weightedCMIM.argtypes = [c.c_uint, c.c_uint, c.c_uint, _uintpp, c.POINTER(c.c_uint), c.POINTER(c.c_double), c.POINTER(c.c_uint), c.POINTER(c.c_double)]
...
class weightedCMIM(BaseEstimator, TransformerMixin, Transformer):
    ...
    def fit(self, data, labels):
        ...
        c_no_of_samples = c.c_uint(n_samples)
        c_no_of_features = c.c_uint(n_features)
        c_feature_matrix = (data.__array_interface__['data'][0] + np.arange(data.shape[1]) * (data.strides[1])).astype(np.uintp)
        c_class_column = labels.ctypes.data_as(c.POINTER(c.c_uint))
        c_output_features = output.ctypes.data_as(c.POINTER(c.c_uint))
        c_weight_vector = weight_vector.ctypes.data_as(c.POINTER(c.c_double))
        c_feature_scores = selected_features_score.ctypes.data_as(c.POINTER(c.c_double))
        ...
        c_selected_features = self.libFSToolbox.weightedCMIM(c_k, c_no_of_samples, c_no_of_features, c_feature_matrix, c_class_column, c_weight_vector, c_output_features, c_feature_scores)
